...
pub struct GraphicsMeshes {
    ...
    #[serde(default)]
    pub default_profile_witnesses: Vec<String>,
}
...
pub fn graphics_with_resolver_at_detail<'a>(
    ...
    let mut out = GraphicsMeshes {
        ...
        default_profile_witnesses: selection.default_profile_witnesses,
        ..Default::default()
    };
...
mod tests {
    ...
    fn graph() -> ObjectGraph {
        let v: Value = serde_json::from_str(include_str!(
            "../tests/fixtures/saved-planar-floor-2024.json"
        ))
        .unwrap();
        serde_json::from_value(v["graph"].clone()).unwrap()
    }
    #[test]
    fn older_saved_identifier_wrapper_preserves_face_material_references() {
        let mut g = graph();
        let expected = graphics(&g);
        for o in &mut g.objects {
            if o.class_name == "Face" {
                let id = crate::native_metadata::identifier(&o.fields["m_renderStyleId"]).unwrap();
                o.fields["m_renderStyleId"] = serde_json::json!({"m_id":{"m_id":id}});
            }
        }
        let actual = graphics(&g);
    ...
    #[test]
    fn indexed_pointer_preserves_duplicate_and_anonymous_refusals() {
        let mut g = graph();
        let e = g.edges[0].clone();
        let v = serde_json::json!({"pointer_token":e.pointer_token,"offset":e.pointer_offset,"class_tag":e.target_class_tag});
        assert_eq!(
            PointerIndex::new(&g)
                .resolve(e.source_object_index, &v)
                .unwrap(),
            pointer(&g, e.source_object_index, &v).unwrap()
        );
        ...
        assert!(pointer(&g, e.source_object_index, &v).is_err());
        let anonymous = serde_json::json!({"pointer_token":u32::MAX,"offset":usize::MAX});
        assert!(PointerIndex::new(&g).resolve(0, &anonymous).is_err());
    }
    #[test]
    fn saved_floor_closed_volume_and_outward_normals() {
        ...
        assert_eq!(tris, 12);
        assert!((volume - 147. * 89. / 6.).abs() < 1e-7, "{volume}");
    }
    #[test]
    fn disconnected_boundary_is_refused() {
        let mut g = graph();
        let e = g
            .objects
            .iter_mut()
            .find(|o| o.class_name == "Edge")
            .unwrap();
        e.fields["m_firstAndLastEdgePnts"][0]["uv"][0][0] = serde_json::json!(12345.);
        assert!(!graphics(&g).diagnostics.is_empty());
    }
    #[test]
    fn unsupported_surface_is_refused() {
        ...
        g.objects
            .iter_mut()
            .find(|o| o.class_name == "Plane")
            .unwrap()
            .class_name = "Cylinder".into();
        assert!(!graphics(&g).diagnostics.is_empty());
    }
