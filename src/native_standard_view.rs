//! Membership in the exporter-created standard 3D profile.
//!
...
//! Native runtime checks qualify the three physical owner classes below;
...
pub fn evaluate_created_standard_3d(
    ...
    if !matches!(o.class_name.as_str(), "Floor" | "SWall" | "FamilyInstance") {
        return Err("owner class outside supported standard-3D profile".into());
    }
    let f = &o.fields;
    let id = |name: &str| {
        f[name]["m_id"]["m_id64"]
    ...
    let provenance = |visible| {
        json!({
            "method":"created_standard_3d_membership",
    ...
    for field in ["m_ownerDBViewId", "m_designOptionId", "m_demolishedPhaseId"] {
        if id(field)? != -1 {
            return Err(format!("{field} outside supported default-view scope"));
        }
    }
    if id("m_createdPhaseId")? != current_phase_id {
        return Err("phase transition outside supported Show Complete scope".into());
    }
    ...
    if o.class_name == "FamilyInstance" {
        for field in ["m_invisible", "m_instDormant"] {
            if f[field].as_bool() != Some(false) {
                return Err(format!("{field} outside supported family visibility scope"));
            }
        }
    }
