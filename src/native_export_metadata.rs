...
fn omni_class_catalog() -> &'static Value {
    ...
    CATALOG.get_or_init(|| {
        serde_json::from_str(include_str!("data/classification-vocabulary.json"))
            .expect("validated bundled classification vocabulary")
    })
}
...
impl Builder {
    ...
    pub fn ingest(&mut self, record: &Record) -> Result<()> {
        ...
        if let Some((name, branch)) = system_family {
            self.system_family_names.insert(record.identity.element_id,(name.into(),json!({"record":source,"native_getter":branch,"vocabulary_profile":"family_names"})));
        }
            ...
            if let Some(family) = fam {
                if let Some(code) = family.root["m_omniClassCode"].as_str() {
                    ...
                    put(
                        ...
                        title_key.into(),
                        json!(title),
                        json!({"record":entry.source,"family":family.source,"source_field":"Family.m_omniClassCode","code":code,"catalog_profile":catalog["profile"],"interpretation":"exact vocabulary lookup; unknown code returns empty"}),
                    );
                }
            }
