...
pub fn extract(rf: &mut RevitFile) -> Result<Scene> {
    ...
    // Complete schema hash identifies the independently validated registry, not
    // a byte-name search or assumed cross-version numeric tag mapping.
    #[derive(Deserialize)]
    struct ClassProfile {
        name: String,
        tag: u16,
    ...
    if schema_hash != profile.schema_sha256 || version != profile.version {
        scene.status = "unsupported_schema_profile".into();
        scene
            .diagnostics
            .push("Formats/Latest differs from validated named registry profile".into());
        return Ok(scene);
    }
