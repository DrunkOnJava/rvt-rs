...
pub fn single_layer_wall_report(
    ...
    Ok((
        start,
        volume,
        json!({"method":"DDC18.4.3_HostObj_getMaterialAreaVolume_single_constant_layer","area_start_face_history":[2,-1,-1],"area_end_face_history":[1,-1,-1],"native_integration":"1000 left-endpoint slices when absolute side-area difference exceeds 1e-10","physical_closed_shell_volume":false,"side_source":side_source,"width":width,"width_source":width_source}),
    ))
}
...
pub fn direct_shape_type_areas(
    ...
    if !meshes.diagnostics.is_empty()
        || !meshes.default_profile_witnesses.is_empty()
...
pub fn constant_layers_floor_report(
    ...
    Ok((
        result,
        json!({"method":"DDC18.4.3_HostObj_getMaterialAreaVolume_equal_caps_constant_layers","cap_source":cap_source,"layers":layers,"physical_closed_shell_volume":false,"repeated_material_areas_accumulate_per_layer":true}),
    ))
}
