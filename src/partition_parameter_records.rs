...
//! native-Revit observations documented in `tools/oracle/README.md`.
...
mod tests {
    ...
    #[test]
    fn native_2027_measured_parameter_spans() {
        let fixtures: serde_json::Value = serde_json::from_str(include_str!(
            "../tests/fixtures/oracle-2027/parameter-spans.json"
        ))
        .unwrap();
        let mut duplicate_bytes = Vec::new();
        for case in fixtures["cases"].as_array().unwrap() {
            let bytes: Vec<u8> = case["bytes_hex"]
                .as_str()
                .unwrap()
                .as_bytes()
                .chunks_exact(2)
                ...
                .collect();
            if case["variant"].as_str() == Some("duplicate_mark") {
                duplicate_bytes.extend_from_slice(&bytes);
            }
            let values = scan_bounded_string_parameter_occurrences_2027(
                &bytes,
                case["parameter_id"].as_i64().unwrap(),
                2027,
            )
            .occurrences;
            assert_eq!(values.len(), 1, "native case {}", case["variant"]);
            assert_eq!(values[0].value, case["expected_value"].as_str().unwrap());
            if !values[0].value.is_empty() {
                assert!(
        ...
        let duplicates = scan_bounded_string_parameter_occurrences_2027(
            &duplicate_bytes,
            i64::from(INSTANCE_MARK_PARAMETER_ID),
            2027,
        )
        ...
        assert!(duplicates.iter().all(|value| value.value == "MARK_A001"));
    }

    #[test]
    fn native_2027_warning_reference_is_not_a_bounded_string() {
        let case: serde_json::Value = serde_json::from_str(include_str!(
            "../tests/fixtures/oracle-2027/warning-reference-negative.json"
        ))
        .unwrap();
        let bytes: Vec<u8> = case["bytes_hex"]
            .as_str()
