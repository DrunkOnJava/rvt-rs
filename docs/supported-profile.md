...
| Browser viewer | Zero-upload inspection, File Status (storey names + material samples), a scene tree grouped by storey and category with Revit-style type-first element names and curtain-wall and stair parts under their whole, a typed element info panel (name, type, GUID, clickable storey, placement and extents in feet, property sets as unit-carrying rows, host and hosted rows), a schedule grouped by IFC type with per-type scene highlight, and explicit export-readiness labels before download. |
