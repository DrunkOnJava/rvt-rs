...
| [#34](https://github.com/DrunkOnJava/rvt-rs/issues/34) | The IFC's material list is still partition display-name strings, 102 against the export's 10 on Core Interior and far more elsewhere (946 on Snowdon against 220 material records). RE-58 now names the `OST_Materials` records themselves by ElementId (78 of 86 here), so the next step is writing those instead of the strings, and joining family instances to their materials. |
