...
| IFC4 writer | Partial | Synthetic fixtures validate in IfcOpenShell; diagnostic mode can include low-confidence proxy provenance; `rvt-ifc --diagnostics` emits a JSON readiness sidecar | Correct writer path exists, but real-file inputs are incomplete. |
