...
| Decode typed elements from real project files | **Partial** | Production `iter_elements`: ArcWall (2023) + partition MVP Levels/Materials + 2024 ArcWallRectOpening (ElemTable-confirmed related ids) + 2024 partition element records for `OST_Walls` / `OST_Doors` / `OST_Windows` / `OST_Columns` / `OST_Floors` / `OST_BuildingPad` (360/132/6/256/80 IFCSLAB + 20 IFCSHADINGDEVICE on Core Interior, exact ElementId sets, cross-witness gated, #204/#211/#212, RE-21/RE-22); wall bodies join-trimmed and columns joined to their family type (#215, RE-26); Revit 2025 element records decode with a per-release marker (walls 7/7, slabs 2/2, rooms 11/11 on the MIT RE1 model, 0 FP on six 2025 files, RE-32); thirteen more product categories on 2024 and 2025 — furniture, casework, plumbing fixtures, specialty equipment, ceilings, curtain-wall mullions and panels, railings, wall sweeps, ducts, duct fittings, pipes, pipe fittings — with no element outside Revit's exports of the three RE1 models (RE-33); structural framing, structural columns, structural foundations and generic models as `IfcBeam` / `IfcColumn` / `IfcFooting` / `IfcBuildingElementProxy`, every id the Snowdon Towers structural VIM carries under the same category (RE-36, measured locally); beams along the location line their element data carries, a level beam as its section's plan rectangle rotated to the line and a sloped one swept along its centreline, 923 of 942 on Snowdon Towers Structural with Revit's own section on 613 of the 839 a later edition's VIM keeps unchanged and on 179 more less a floor join's cut (RE-49, measured locally; ends are not trimmed at supports); lighting fixtures, air terminals, food-service equipment, planting, parking, entourage, hardscape, elevators and ramps, with no element outside Revit's exports of Snowdon Towers and RE1 Mechanical and Electrical (RE-37); stairs as bodiless `IfcStair` aggregates of their runs, landings and stringers, every relation one Revit's export has (RE-39), and roofs as `IfcRoof` (all 20 Snowdon roofs are Revit's `IfcRoof` Tags), 13 of them with the outline their sketch lines close and 11 of those with the area of Revit's own roof, sketch lines' exact recorded ends closing outlines the box solve leaves ambiguous for roofs and floors alike (RE-50, measured locally; roof slopes are not read); elements in a design option set's non-primary options left out as Revit's export leaves them out, the primary read from the set's name entry, and slab edges as proxies (RE-40: the 29 Snowdon elements in primary options are all in Revit's export, the 27 in others all missing from it); family instances named `Family:Type:ElementId` from the partition name entries, identical to Revit's own names on all 4,397 elements named across Core Interior, Snowdon Towers and the RE1 models (RE-38, with RE-42 telling a family from the families nested in it); elements, rooms and storeys carry the GlobalId Revit's own exporter gives them, rebuilt from `Global/History` and `Global/ElemTable` (every element rvt-rs exports that Revit's export holds: Core 854, RE1 281, Snowdon 5,945; RE-48); storeys carry their Revit Level's name, elevation and GlobalId on 2024 and 2025 projects, from RE-24's name block read with a six-byte per-release elevation marker and a lead-in-confirmed copy, second-prologue Levels attributed through their partition record and Levels inside other elements left out: every storey equals one of Revit's export on Snowdon Towers (18 of 18), each RE1 model (2 of 2), Projeto1 and teste_export_2025, and Core Interior (15 of 15), with 3,379 of 3,380 Snowdon elements in Revit's storey (RE-51; Snowdon's structural model refused, 8 of its Levels carrying an unexplained flag byte); stairs and flights carry `Pset_StairCommon` / `Pset_StairFlightCommon` riser count, riser height and tread length read from their element data, Revit's own on every Snowdon stair (26 of 26) and on 37 of 43 flights (RE-47), and straight flights with separate treads and risers drawn as them from their sketch and run type, 30 of Snowdon's 34 drawn equal to Revit's own geometry (RE-52, measured locally); walls a curtain-wall mullion names export as bodiless `IfcCurtainWall` aggregates of their panels and mullions, exactly Revit's 42 on Snowdon Towers and 1 on RE1, with 1,798 of Revit's 1,906 Snowdon part relations reproduced (RE-46); walls, floors, ceilings, roofs and railings carry their system-family type's name as `TypeName`, read from the type's own element data and Revit's own on 2,028 of 2,029 across Core Interior, Snowdon Towers, RE1, Projeto1 and `teste_export_2025`, whose other wall that export shows in a later state of the model (RE-44, #322); second-prologue records take the ElementId of the partition record they sit in (RE-35; every one of 30,432 records carrying their own id starts a record of that id), which completes every recovered entity on the three RE1 models and takes Snowdon Towers Architectural to 6,045 exported elements, 5,945 of them in Revit's export (every element of Revit's export in the entity types rvt-rs writes, RE-43) and the rest ones Revit's export omits, with none left unattributed once the ElemTable's second id is declared (RE-41); placed instances with no 3D volume are left out as Revit leaves them out (#309), and so are curtain panels of the "Empty System Panel" family, all 24 on Snowdon Towers (recognised by that English family name); HostObjAttr filtered; RE-19 negatives intact: no opening-index Door/Window discriminator, no schema-field Wall on magnetar corpora | Full model conversion is not ready; six categories on one Revit 2024 edge match Revit's exporter exactly, spaces and materials do not. |
