...
- **Geometry extraction is Phase 5**. The IFC export produces valid placement and extruded rectangular solids when the caller supplies dimensions. The reader now recovers ArcWall ElementIds, base elevations, elevation-derived storeys, and Z-delta heights for the Revit 2023 Einhoven record layout (thickness remains unresolved until WallType width join); arbitrary wall layouts, 2024+ ArcWall records, floors, doors, windows, profile shapes, and brep geometry are still pending. The Extrusion helpers in [`from_decoded.rs`](../src/ifc/from_decoded.rs) remain the general bridge for caller-supplied dimensions.
