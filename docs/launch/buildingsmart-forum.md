...
- **Unit recovery is best-effort, not exhaustive.** Revit stores units as `autodesk.unit.*` identifiers (e.g. `autodesk.unit.unit:millimeters-1.0.1`). The exporter now recovers modal unit identifiers from `Partitions/NN`, maps the common length / area / volume / angle / mass / time units it understands, and preserves unmapped unit identifiers in diagnostics. The long tail of compound MEP / structural / temperature / currency units still needs review against buildingSMART and IfcOpenShell conventions before I call it complete.
