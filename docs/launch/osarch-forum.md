...
- **IFC-40 — Review Revit unit mapping (`autodesk.unit.*` identifiers) to `IfcSIUnit` / `IfcConversionBasedUnit`.** The exporter now recovers modal project unit identifiers from `Partitions/NN` and maps the common length / area / volume / angle / mass / time units it understands. What I still do not want to get wrong is the long tail: `autodesk.unit.unit:fahrenheit-1.0.1`, `autodesk.unit.unit:poundsForceFeet-1.0.1`, and the hundreds of compound MEP / structural units. A review against what IfcOpenShell and BlenderBIM import cleanly would save a lot of trial-and-error.
