...
This is a portable implementation reference for the observed Revit native
format. It condenses the larger forensic specification in the SpatialLogic
research workspace into version-qualified rules that can guide `rvt-rs`
maintenance without requiring private models or oracle artifacts.

The observations cover multiple Revit file families and releases. They are not
an Autodesk normative specification. A structural match establishes how a
particular stream or record is framed; it does not establish that every class,
field, or semantic API object has the same encoding in every release.

The runtime must keep three statements separate:

1. bytes were read and structurally decoded;
2. a native object graph or field was projected with provenance;
3. an evaluated Revit API semantic result was independently qualified.

The first does not imply the second, and the second does not imply the third.
See [native projection provenance](../native-provenance.md) for the common
status and source-chain contract.
...
The CFB layer is comparatively stable in the measured 2016–2027 corpus. Inner
...
measured page layout is stream-sensitive. Preparation must be named and
...
stripping rule because observed stripping reduced recovered schema content.
The runtime does not claim to implement a universal error-correction or
whole-file checksum algorithm. CFB transaction signatures are not a substitute
for a content hash; source reproducibility uses an external cryptographic hash
of the input file.
...
prepared length, opaque suffix length, and hash where available.
...
Observed field categories include signed and unsigned scalar values, strings,
...
on the measured corpus, but a classified field type does not prove that every
...
See the existing native research checkpoint and parser references in
...
are retained as uninterpreted flags. The current registry accepts only the
measured base/modifier combinations, bounds recursive depth and field count,
...
Two measured special cases are deliberately narrow. A `ClassDefinitionRef`
...
`Global/ElemTable` has multiple observed record families. The measured forms
...
The corrected measured 2027 `ElemRec` array begins at the schema-derived array
...
The 2024 40-byte profile also has a tested `u32` at `+0x1c` that behaved as a
monotone version discriminator across 976 repeated frames. Its use is gated to
that profile. Counts and candidate frames are validation constraints; they are
not permission to apply these offsets to another document kind or release.
...
Graph object indexes and serialized pointer offsets are therefore part of
provenance whenever a field is projected.
...
The narrow partition element-record profiles are version and corpus gated.
...
Observed API storage categories include strings, integers, doubles, and
ElementId-valued references. Modern files also retain spec, unit, and parameter
group identifiers in measured profiles.
...
schema hash matches the measured profile; it supplies definitions, not owner
...
the Revit API. The bounded native reader handles two measured catalog wrappers:
...
represents a serialized-absent entity. The measured present form uses
...
ES references must remain distinct from BIM topology and ElemTable ownership.
They must not be emitted as default IFC relationships or canonical host edges
without an independent evidence gate. Unsupported schema wrappers, payload
...
for the saved-graphics boundary and the existing appearance research links in
...
1. validate CFB structure and retain the source hash;
...
7. project typed observations with source paths and refusal diagnostics;
8. run independent validation only where an API or owned fixture exists.
...
The native implementation is opt-in and parallel to the legacy walker/exporter.
The public entrypoints are [`rvt-native-document`](../../src/bin/rvt_native_document.rs),
[`rvt-native-scene`](../../src/bin/rvt_native_scene.rs),
[`rvt-native-world-model`](../../src/bin/rvt_native_world_model.rs), and
[`rvt-native-saved-scene`](../../src/bin/rvt_native_saved_scene.rs). Representative
probes are [`research_native_coverage`](../../examples/research_native_coverage.rs),
...
[`research_saved_scene`](../../examples/research_saved_scene.rs). They emit
source-qualified observations and diagnostics; a successful structural read is
not a claim of universal typed recovery, regeneration, visibility parity, or
render parity.
