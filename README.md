...
| Gap | Status | Evidence |
|---|---|---|
| Element extraction from real `.rvt` project files | **partial** | ArcWall (2023) + partition MVP Level/Material/Room (+ Floor plan-loops where no element records decode) + 2024 ArcWallRectOpening index + 2024 partition element-record Wall/Door/Window/Column/Floor/BuildingPad instances (exact ElementId sets vs Revit's own export, #204/#211/#212) merge into production `iter_elements`. Opening related ids are ElemTable-confirmed (not Door/Window). All 80 exported slabs also carry the plan profile their `OST_SketchLines` records close (#31/RE-25); wall bodies are cut back by the joins their reference list names and column bodies are the prism minus the walls that cut them — 351 of 360 walls and 256 of 256 columns world-exact (#215/#239, RE-26 and RE-29). Storey containment comes from the Level ElementId the element record names: 853 of 872 exported elements land on a specific storey and the other 19 are contained in the `IfcBuilding` rather than filed under an arbitrary one (#219/RE-27). Doors and windows are bound to their host wall (#222/RE-23); schema-field Walls remain unsolved. All of that is measured on `2024_Core_Interior.rvt`: on Autodesk's Snowdon Towers 2024 samples most element records use a second layout whose ElementId is not located, so 7 of 2,171 architectural wall/door/window/column/floor/room records decode, and the export diagnostics count the rest as `element_record_without_element_id` with a warning that the model is incomplete (RE-30). Production path filters HostObjAttr; diagnostic API retains broad candidates. See `reports/element-framing/RE-01-synthesis.md`. |
