...
| Gap | Status | Evidence |
|---|---|---|
| Element extraction from real `.rvt` project files | **partial** | ArcWall (2023) + partition MVP Level/Material/Room/Floor plan-loops + 2024 ArcWallRectOpening index + 2024 partition element-record Wall/Door/Window/Column instances (exact ElementId sets vs Revit's own export, #204/#211) merge into production `iter_elements`. Opening related ids are ElemTable-confirmed (not Door/Window). Schema-field Walls, Door/Window host IFC binding and slab instance identity (#212) remain unsolved. Production path filters HostObjAttr; diagnostic API retains broad candidates. See `reports/element-framing/RE-01-synthesis.md`. |
