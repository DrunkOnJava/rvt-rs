...
| Element geometry beyond boxes and sketched outlines | partial | A body is the element record's bounding box, refined where the bytes allow it: slabs take their sketched plan profile, wall runs are trimmed at their joins, and columns lose what the walls cut from them. No curved, sloped or B-rep geometry is decoded. Doors and windows are boxes. A slab sketched as separate pieces keeps its box (#331). |
