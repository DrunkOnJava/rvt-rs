...
**Generic real-project typed element extraction is mostly unsolved**, with narrow partition MVP exceptions. Production `walker::iter_elements` prefers typed MVP decoders on `Global/Latest` (fail closed), merges version-gated 2023 ArcWall partition recovers, and merges fail-closed partition MVP recovers for Level / Material plus 2024 ArcWallRectOpening index rows (corpus-proven on magnetar Einhoven / Core Interior). Opening related ids are ElemTable-confirmed (still not typed Door/Window). On Revit 2024 it additionally merges partition *element records*, whose header names the element's `BuiltInCategory` outright: `OST_Walls` / `OST_Doors` / `OST_Windows` / `OST_Columns` records that carry no container reference and are marked placed instances reproduce Revit's own exported ElementId sets exactly — 360 `IfcWall`, 132 `IfcDoor`, 6 `IfcWindow`, 256 `IfcColumn` on `2024_Core_Interior.rvt`, tolerance 0, cross-witness gated (#204 / #211, `reports/element-framing/RE-21-partition-element-record-instance-rule.md`). Bodies are the record's bounding box, except for the plan profile of a slab, which is the sketch boundary its `OST_SketchLines` records close (#31, `reports/element-framing/RE-25-slab-plan-profiles.md`), and the plan run of a wall, which is the box cut back by the joins its own reference list names — 351 of 360 walls then match Revit's world envelope exactly, up from 27, with 9 ends at true L corners left over-trimmed as a measured negative, and a column body is the record prism minus the record prisms of the walls that cut it, exact on 256 of 256 at a worst residual of 9.4e-13 ft (#215 / #238 / #239, `reports/element-framing/RE-26-world-coordinate-residuals.md` and `reports/element-framing/RE-29-join-reference-list.md`). Every wall also names exactly one wall-type record, which is the `IfcWallType.Tag` the export assigns on 360 of 360 (#88, `reports/element-framing/RE-28-wall-type-records.md`); that join is library-side and is not yet written onto the emitted wall. Doors and windows are bound to their host wall (#222, RE-23); schema-field Walls remain open — diagnostic scans still find `HostObjAttr`-style candidates on `Global/Latest` for those classes. **81** per-class decoder structs ship in `elements::all_decoders()`; `MVP_TYPED_CLASSES` are consulted by `iter_elements`, while the broader registry remains a library building block. Root-cause investigation (`reports/element-framing/RE-01-synthesis.md`) found that element instance data lives in `Partitions/*` streams with a wire envelope that has only been reverse-engineered for ArcWall (2023) and opening-index (2024) subsets. Q-01 community-corpus open/scaffold validation has been run (`docs/corpus-hunt-2026-04-21.md`: 222/223 real files pass open → schema → scaffold IFC); that is not full typed model recovery. See [What does not work yet](#what-does-not-work-yet) below.
...
| Gap | Status | Evidence |
|---|---|---|
| Element extraction from real `.rvt` project files | **partial** | ArcWall (2023) + partition MVP Level/Material + 2024 ArcWallRectOpening index + 2024 partition element-record Wall/Door/Window/Column/Floor/BuildingPad instances (exact ElementId sets vs Revit's own export, #204/#211/#212) merge into production `iter_elements`. Opening related ids are ElemTable-confirmed (not Door/Window). All 80 exported slabs also carry the plan profile their `OST_SketchLines` records close (#31/RE-25); wall bodies are cut back by the joins their reference list names and column bodies are the prism minus the walls that cut them — 351 of 360 walls and 256 of 256 columns world-exact (#215/#239, RE-26 and RE-29). Storey containment comes from the Level ElementId the element record names: 853 of 872 exported elements land on a specific storey and the other 19 are contained in the `IfcBuilding` rather than filed under an arbitrary one (#219/RE-27). Doors and windows are bound to their host wall (#222/RE-23); schema-field Walls remain unsolved. Revit 2025 files decode the same way with a per-release record marker: walls, slabs and rooms match Revit's own export with no false positives on six 2025 files, with room names and storeys still 2024-only (RE-32). On both releases thirteen more categories decode the same way, each exported element being one Revit's own export also holds: furniture, casework, plumbing fixtures, specialty equipment, ceilings, curtain-wall mullions and panels, railings, wall sweeps, ducts, duct fittings, pipes and pipe fittings (RE-33; all 58 on the RE1 architecture model, and no element outside Revit's export on the three RE1 models). Structural framing, structural columns, structural foundations and generic models follow (RE-36): on Autodesk's Snowdon Towers structural sample every one of them its VIM export carries has the same category there (914 framing members, 74 columns, 90 foundations, 91 generic models), and they export as `IfcBeam`, `IfcColumn`, `IfcFooting` and `IfcBuildingElementProxy`. Stairs export as Revit does: a bodiless `IfcStair` aggregating its runs, landings and stringers, every relation one Revit's own export has (RE-39), and roofs as `IfcRoof`. Elements in a design option set's non-primary options are left out, as Revit's export leaves them out: the set's name entry names its primary option (RE-40), and slab edges export as proxies. Lighting fixtures, air terminals, food-service equipment, planting, parking, entourage, hardscape, elevators and ramps follow with no element outside Revit's own exports (RE-37: 447 lighting fixtures on Snowdon Towers, 12 on RE1 Electrical, 7 air terminals on RE1 Mechanical). Family instances carry Revit's own names: the partition name entries give each element's type and family, and the IFC `Name` is `Family:Type:ElementId`, identical to Revit's on all 4,397 elements named across Core Interior, Snowdon Towers and the RE1 models (RE-38), including families that nest others of their category, whose own record names the nested ones (RE-42). The 2024 results are measured on `2024_Core_Interior.rvt`: on Autodesk's Snowdon Towers 2024 samples, and on 2025 projects saved incrementally, most element records use a second layout with no ElementId at `+0x00` (RE-30). Every partition opens with a chain of records, one per element, whose header carries the ElementId, and those records sit inside it, so their id is read from the enclosing record (RE-35). All 30,432 records that do carry their id start a record of the same id. On the RE1 models every recovered entity now matches Revit's own export, and the Snowdon architectural model exports 6,045 elements, 5,945 of them in Revit's export: every element of Revit's export in the entity types rvt-rs writes, once frames holding the invalid ElementId take their record's id (RE-43). The rest are elements Revit's export leaves out (#309); the 18 that were in non-primary design options are now left out too (RE-40). Where an ElemTable record's two ids differ, the second is the element's ElementId, so every wall, column and generic model of Revit's export is now attributed and nothing on that file is left unattributed (RE-41). Placed instances with no 3D volume (2D symbol families) are left out as Revit leaves them out, and are reported as `element_record_without_volume`; the export diagnostics count what remains unattributed as `element_record_without_element_id` with a warning that the model is incomplete. Production path filters HostObjAttr; diagnostic API retains broad candidates. See `reports/element-framing/RE-01-synthesis.md`. |
...
What still depends on decoder / partition-stream work: schema-field Walls, typed Door/Window host IFC, Level ElementId storey assignment for Rooms, door and window bodies, and compound wall layers — whose thicknesses are not within 64 KiB of the wall-type record and which the paired reference export cannot witness at all, since it is a `ReferenceView_V1.2` file carrying zero `IfcMaterialLayerSet` (#88/RE-28), so nothing emits a layer (slab thickness, plan profile and Floor↔ElemTable id binding are closed for the record-backed slab set, #212/#31; wall thickness and the join-trimmed run are closed for the record-backed wall set, #215/RE-26/RE-29). Partition MVP already recovers Level / Material names (and 2023 ArcWall geometry on the research path). **RE-19 / RE-20 (2026-08-29) closed negative** on magnetar corpora — no Door vs Window discriminator *in the opening-index bytes*, no schema-field/2024 Wall envelope, and no recoverable Level ElementId map — so stop re-probing those ids until new corpus or signals appear. RE-21 / RE-22 (2026-08-30) are a **different carrier** and do not re-open them: the partition element record names the `BuiltInCategory` itself, and the per-element IFC export-type override is a parameter string, not an opening-index or schema-field signal. Treat IFC export as scaffold-plus-diagnostics except on the narrow version-gated paths in [`docs/status.md`](docs/status.md) and [`docs/supported-profile.md`](docs/supported-profile.md).
