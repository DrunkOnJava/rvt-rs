...
|---|---|---|
| Walls | `IfcWall`, runs trimmed at their joins; a 2024 or 2025 wall its type's thickness either side of its centreline | Core Interior 360 of 360 (351 world-exact); Snowdon Towers 1,078 of 1,078, faces exactly Revit's on 1,002 of them (RE-54) |
...
| Storeys | `IfcBuildingStorey` with its Revit Level's name and elevation | Core Interior 15, Snowdon Towers 18, every RE1 model 2; name, elevation and GlobalId all Revit's (RE-24, RE-51) |
| Storey containment | each element in the storey of the Level its record names, of its base constraint where it names two, and, where it names none, of a railing's host or of the Level its work planes carry | Core Interior all 854 in Revit's storey; Snowdon Towers 5,805 in Revit's storey, 44 in another and 102 on none (RE-27, RE-59, RE-60) |
| Names | `Family:Type:ElementId`, `ObjectType` `Family:Type`, for family instances and, since RE-63 and RE-64, walls, curtain walls, floors, ceilings, roofs, railings, slab edges, ramps and curtain panels that are walls | 4,397 family instances named exactly as Revit names them; Snowdon's 1,077 walls, 20 roofs and 68 ceilings and Core Interior's 460 system-family elements all named as Revit names them (RE-63); Snowdon's 18 panel walls and 59 slab edges too (RE-64) |
...
Across Autodesk's Snowdon Towers architectural sample (Revit 2024), rvt-rs exports 6,045 elements. 5,945 of them are in Revit's own export, which is every element Revit exports in these entities. The other 100 are elements Revit's exporter leaves out, such as empty curtain panels and opening-cut families (#309). Elements in a design option set's non-primary options and 2D-only families are left out, as Revit leaves them out, and the export diagnostics count them.
...
| Gap | Status | Evidence |
|---|---|---|
| Element extraction from real `.rvt` project files | **partial** | Revit 2024 and 2025 projects decode their element records into typed IFC elements; see [What rvt-rs reads from real projects](#what-rvt-rs-reads-from-real-projects). Earlier releases open and report metadata and schema, but their element records use a different prologue that is not decoded (only Revit 2023 ArcWalls are). The per-finding history, RE-21 to RE-43, is in [`docs/status.md`](docs/status.md) and `reports/element-framing/`. |
...
| Element geometry beyond boxes and sketched outlines | partial | A body is the element record's bounding box, refined where the bytes allow it: slabs take their sketched plan profile, wall runs are trimmed at their joins, and columns lose what the walls cut from them. No curved, sloped or B-rep geometry is decoded. Doors and windows are boxes. A slab sketched as separate pieces exports one element per piece, as Revit's export does (#331). |
