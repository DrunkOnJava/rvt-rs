...
| Gap | Status | Evidence |
|---|---|---|
| Element extraction from real `.rvt` project files | not functional | Scanner finds 1/405 classes on Einhoven 2023, 0/395 on 2024 Core Interior. Real Walls/Floors/Doors are not returned by `iter_elements` on any real project file tested. Root cause: wrong stream scanned (should be `Partitions/*`, not `Global/Latest`). RE needed. See `reports/element-framing/RE-01-synthesis.md`. |
...
| IFC4 typed elements from real `.rvt` | not functional | `RvtDocExporter::export` on a real `.rvt` emits framework entities + `IFCBUILDINGELEMENTPROXY` placeholders, not typed `IfcWall` / `IfcDoor` / etc. Typed IFC output is only visible against synthesized fixtures. |
...
- **As scaffolding, not functional on real project files yet** — `iter_elements` walker, walker→IFC integration. On real `.rvt` project files these return `HostObjAttr` proxies rather than typed elements (see "What does not work yet" above).
