...
use rvt::{compression, formats, object_graph, streams, RevitFile};
...
fn main() -> anyhow::Result<()> {
    let path = std::env::args().nth(1).expect("usage: adocument_entry <file>");
    let mut rf = RevitFile::open(&path)?;

    // Find where the history-string section ends.
    let history = object_graph::DocumentHistory::from_revit_file(&mut rf)?;
    println!("Document upgrade history: {} entries", history.entries.len());
    println!("  string section begins at 0x{:x}", history.string_section_offset);
    ...
    loop {
        if scan + probe.len() >= d.len() { break; }
        let slice_end = (scan + 512).min(d.len());
        match d[scan..slice_end].windows(probe.len()).position(|w| w == probe) {
            Some(p) => {
                let pos = scan + p;
                let mut end = pos;
                while end + 2 <= d.len() {
                    let c = u16::from_le_bytes([d[end], d[end + 1]]);
                    if c < 0x20 && c != b' ' as u16 { break; }
                    end += 2;
                }
                last_revit_tail = end;
                scan = end + 1;
    ...
    if let Some(adoc) = adoc {
        println!("\nADocument schema: @0x{:x}, {} fields declared", adoc.offset, adoc.fields.len());
        for f in adoc.fields.iter().take(13) {
            let ft = f.field_type.as_ref().map(|ft| format!("{ft:?}")).unwrap_or("None".into());
            println!("  {} :: {}", f.name, ft);
        }
        ...
        for f in &adoc.fields {
            if let Some(ft) = &f.field_type {
                let sz = match ft {
                    ...
                    formats::FieldType::Unknown { .. } => { unknown_count += 1; 0 }
                };
                expected_size += sz;
            }
        }
