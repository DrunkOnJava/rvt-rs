//! Capture one ESEntity framing failure plus schema/global context; research only.
...
fn main() -> Result<()> {
    ...
    let global = native_document::read_single(&mut file, "Global/Latest")?;
    fs::write(out.join("global-latest.bin"), &global)?;
    let global_graph = native_parameters::decode_graph_with_limits(
        &global,
        &registry,
    ...
    fs::write(
        out.join("global-graph.json"),
        serde_json::to_vec_pretty(
            &json!({"graph":global_graph.as_ref().ok(),"error":global_graph.as_ref().err().map(|e|format!("{e:#}"))}),
        )?,
    )?;
    let tag = registry
        .named(&args[2])
        ...
        .filter(|s| s.starts_with("Partitions/"))
    {
        ...
        let walked = native_segments::walk(
            &prepared,
            &registry,
            256 * 1024 * 1024,
            |source, bytes| {
                ...
                while pos < bytes.len() {
                    ...
                    if body.len() >= 2
                        && u16::from_le_bytes(body[..2].try_into()?) == tag
                        && native_parameters::decode_graph(body, &registry)
                            .err()
                            .is_some_and(|e| {
                                format!("{e:#}").contains("ES entity")
                                    || format!("{e:#}").contains("ESEntity.m_blob")
                            })
                    {
                        ...
                        anyhow::bail!("bounded capture complete");
                    }
                    count += 1;
                    total += len as u64;
                    pos = end + 4;
